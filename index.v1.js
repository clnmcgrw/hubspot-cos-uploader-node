...
module.exports = function(options) {
	...
	ROOT = options.root || './hubspot',

	ROOTFILES = Array.isArray(ROOT) ? ROOT.map(item => `${item}/*.{html,css,js}`) : `${ROOT}/*.{html,css,js}`;

	//public api props
	this.watcher = null; //expose chokidar methods
	...
	createNewTemplate = function(localPath) {
		var html = fs.readFileSync(localPath).toString(),
				...
				filename = getFilenameFromPath(localPath);

		if (!filemeta) {
			logger(chalk.red, `Check that the [hubspot-metadata] JSON for ${filename} is formatted correctly.`);
			return;
		}
	...
	updateRemoteTemplate = function(localPath) {
		...
		var html = fs.readFileSync(localPath).toString(),
				...
				filename = getFilenameFromPath(localPath);
		if (!filemeta) {
			logger(chalk.red, `Check that your [hubspot-metadata] JSON for ${filename} is formatted correctly.`);
			return;
		} 
		if (!fileNamesMatch(localPath, filemeta.path)) {
			logger(chalk.red, 'The name of your local file should match the "path" filename in your metadata.');
		...
		if (!filemeta.id) {
			...
			logger(chalk.yellow, `File ${filename} has no ID in its metadata - creating as new template.`);
	...
	startWatcher = function() {
		...
		this.watcher = chokidar.watch(ROOTFILES);

		this.watcher.on('change', updateRemoteTemplate);
		this.watcher.on('add', createNewTemplate);

		this.watcher.on('error', error => logger(chalk.red, `File watcher error: ${error}`));
		this.watcher.on('ready', () => logger(chalk.green, 'File watchers started.'));
	};
